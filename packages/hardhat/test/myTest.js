...
describe("🚩 Challenge 2: 🏵 Token Vendor 🤖", function () {
  ...
  if(process.env.CONTRACT_ADDRESS){
    it("Should connect to external contract", async function () {
      ...
      console.log(`\t`,"📡 Loading the token address from the Vendor...")
      console.log(`\t`,"⚠️ Make sure *token* is public in the Vendor.sol!")
      let tokenAddress = await vendor.token();
  ...
  describe("💵 buyTokens()", function () {
    it("Should let us buy tokens and our balance should go up...", async function () {
      ...
      const startingBalance = await token.balanceOf(owner.address)
  ...
  describe("💵 sellTokens()", function () {
    it("Should let us sell tokens and we should get eth back...", async function () {
      ...
      const approveTokensResult = await token.approve(vendor.address, ethers.utils.parseEther("0.1"));
